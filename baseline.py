...
class BaselineManager:
    ...
    def load(self) -> dict:
        try:
            ...
            logger.info("Baseline loaded from s3://%s/%s", self.bucket, self.baseline_key)
            return baseline
    ...
    def update(self, baseline: dict, channel: str, new_values: list[float]) -> dict:
        ...
        try:
            ...
            logger.info(
                "Baseline updated -- channel: %s | observations: %d -> %d | mean: %.4f | std: %.4f",
                channel, prev_count, state["count"], state["mean"], state["std"]
            )
